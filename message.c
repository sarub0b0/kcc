...
void verror_at(char *loc, char *input, char *filename, int line_num, char *fmt,
               va_list ap) {
  char *line = loc;
...
void error_at(char *loc, char *fmt, ...) {
  int line_num = 1;
  for (char *p = current_user_input; p < loc; p++)
    if (*p == '\n')
      line_num++;
  ...
  verror_at(loc, current_user_input, current_filename, line_num, fmt, ap);
